...
public class RunTime_Terrain_Convertion : MonoBehaviour 
{
    ...
    Material GenerateMaterial_Basemap()
    {
        ...
        {
            Shader srpShader = Shader.Find("Universal Render Pipeline/Lit");
            if(srpShader == null)
    ...
    Material GenerateMaterial_Splatmap()
    {
        ...
        else if (usedTexturesCount == 1)
        {
            ...
            if (UnityEngine.Rendering.GraphicsSettings.renderPipelineAsset == null)
            {
                ...
                if (shader != null)
                {
                    ...
                    //Texture
                    newMaterial.SetTexture("_MainTex", diffuseTextures[0]);

                    //Scale & Offset
                    newMaterial.SetTextureScale("_MainTex", uvScale[0]);
                    newMaterial.SetTextureOffset("_MainTex", uvOffset[0]);
                }
            }
            else
            {
                ...
                if (shader != null)
                {
                    ...
                    //Texture
                    newMaterial.SetTexture("_BaseMap", diffuseTextures[0]);

                    //Scale & Offset
                    newMaterial.SetTextureScale("_BaseMap", uvScale[0]);
                    newMaterial.SetTextureOffset("_BaseMap", uvOffset[0]);
                }
            }
        ...
        {
            shaderName = string.Format("VacuumShaders/Terrain To Mesh/Universal Render Pipeline/Lit" + "/{0} Textures", usedTexturesCount);
        }
        ...
        for (int i = 0; i < usedTexturesCount; i++)
        {
            ...
            //Metalic & Gloss
            newMaterial.SetFloat(string.Format("_V_T2M_Splat{0}_Metallic", i + 1), 0);// metalic[i]);
            newMaterial.SetFloat(string.Format("_V_T2M_Splat{0}_Glossiness", i + 1), 0);// smoothness[i]);
